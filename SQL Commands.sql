...
-- Diffrance beetween Primary key and Foreign key 
...
-- Foreign Key: Enforces a link between two tables. It is a column (or a set of columns) in one table that refers to the primary key in another table. A foreign key can have duplicate values and NULL values.A table can have multiple foreign keys.
...
SELECT city FROM cities WHERE city ILIKE 'a%a'LIMIT 4;
---------------------------------------------------------------------------------
